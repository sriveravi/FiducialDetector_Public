...
function [ X2 Y2 cropPosUnpadded] = cropFiducialMissing( X, Y, coordIndicesShape, cropSize, maskType, ...
   ...
   for i1 = 1:N
    ...
    %        size(X2(:,:,1))
            ...
            if debugCropFiducial
                clf(gcf);
                imagesc( X2(:,:, i1)), axis off equal, colormap(gray);
                hold on
                plot( real( Y2(:,i1)), imag( Y2(:,i1)), 'g*', 'markersize', 9);
                title(int2str(i1));             
                pause(.2);
            end
        end       
    ...
    % remove missing data
    missingMarkings = find(isnan(Y));
    X2(:,:,missingMarkings) = [];
    Y2(:, missingMarkings) = [];
    cropPosUnpadded(:,missingMarkings) = [];
